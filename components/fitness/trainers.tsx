...
import { Star, Award, Users, Calendar, Instagram, Twitter, Linkedin } from "lucide-react"
            ...
            >
              <Card className="bg-navy-800/80 backdrop-blur-sm border border-blue-900/50 overflow-hidden hover:shadow-2xl hover:shadow-blue-500/20 transition-all duration-500 group h-full rounded-3xl">
                ...
                <div className="p-6">
                  ...
                  <motion.button
                    className="w-full text-blue-400 hover:text-blue-300 text-sm transition-colors duration-300"
                    onClick={() => setSelectedTrainer(selectedTrainer === index ? null : index)}
                    whileHover={{ scale: 1.02 }}
                  >
                    {selectedTrainer === index ? "Hide Details" : "View Profile"}
                  </motion.button>

                  <motion.div
                    initial={false}
                    animate={{
                      height: selectedTrainer === index ? "auto" : 0,
                      opacity: selectedTrainer === index ? 1 : 0,
                    }}
                    transition={{ duration: 0.3 }}
                    className="overflow-hidden"
                  >
                    <div className="pt-4 mt-4 border-t border-blue-800/50 rounded-3xl">
                      <p className="text-gray-300 text-sm mb-4">{trainer.bio}</p>

                      <div className="mb-4">
                        <h5 className="text-sm font-semibold text-blue-300 mb-2">Certifications:</h5>
                        <ul className="space-y-1">
                          {trainer.certifications.map((cert, i) => (
                            <li key={i} className="text-xs text-gray-300 flex items-center rounded-xl">
                      ...
                      <div>
                        <h5 className="text-sm font-semibold text-blue-300 mb-2">Achievements:</h5>
                        <ul className="space-y-1">
