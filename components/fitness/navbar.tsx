...
import { Dumbbell, Menu, X, Flame, Calendar, Users, Apple, MessageCircle } from "lucide-react"
...
export default function FitnessNavbar({ onBackToSelection, onBookNowClick }: FitnessNavbarProps) {
  ...
  const navItems = [
    ...
    { 
      name: "Schedule", 
      href: "#schedule", 
      icon: <Calendar className="w-4 h-4 text-blue-400 group-hover:text-emerald-500 transition-colors duration-300" />,
    ...
    { 
      name: "Contact", 
      href: "#contact", 
      icon: <MessageCircle className="w-4 h-4 text-blue-400 group-hover:text-cyan-500 transition-colors duration-300" />,
      color: "text-cyan-500",
      target: "_self"
    }
  ]
            ...
            <div className="flex items-center space-x-3">
              <motion.button
                onClick={() => {
                  ...
                  window.dispatchEvent(new CustomEvent('showModeSwitchPopup', { 
                    detail: { mode: 'fitness' } 
                  }));
                }}
